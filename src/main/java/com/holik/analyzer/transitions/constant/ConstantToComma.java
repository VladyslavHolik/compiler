...
@Component
public class ConstantToComma implements Transition {
    ...
    public Optional<ParseError> getError(List<LexemeValue> lexemes, Integer currentLexemeIndex) {
        if (!functionAnalyzer.isFunctionOpen(lexemes.subList(0, currentLexemeIndex + 1))) {
            ...
            return Optional.of(new ParseError(index, "Comma is not allowed, because constant is not function argument"));
        } else {
